        ...
        if os.environ.get("GOOGLE_APPLICATION_CREDENTIALS"):
            try:
                ...
                api_endpoint = (
                    ...
                    if location != "global"
                    else "documentai.googleapis.com"
                )
                ...
                print(
                    f"[DocumentAIService] Failed to initialize Document AI client: {e}. Running in simulation/fallback mode."
                )
        else:
            print(
                "[DocumentAIService] GCP credentials not set. Running in simulation/fallback mode."
            )
    ...
    def classify_document(
        ...
        if self.enabled and settings.docai_classifier_processor_id:
            try:
                ...
                print(
                    f"[DocumentAIService] Document Classification API call failed: {e}"
                )
    ...
    def extract_layout(self, file_content: bytes, mime_type: str) -> dict:
        ...
        if self.enabled and settings.docai_ocr_processor_id:
            try:
                ...
                request = documentai.ProcessRequest(
                    name=settings.docai_ocr_processor_path, raw_document=raw_document
                )
                ...
                for idx, page in enumerate(document.pages):
                    ...
                    for paragraph in page.paragraphs:
                        para_text = "".join(
                            [
                                document.text[seg.start_index : seg.end_index]
                                for seg in paragraph.layout.text_anchor.text_segments
                            ]
                        ).strip()
                        if para_text:
        ...
        # Fallback simulation
        return {"text": "", "page_count": 0, "tables": [], "page_texts": []}
            ...
            try:
                ...
                for entity in document.entities:
                    ...
                    if entity.page_anchor and entity.page_anchor.page_refs:
                        ...
                        if (
                            pref.bounding_poly
                            and pref.bounding_poly.normalized_vertices
                        ):
                    ...
                    tags.append(
                        {
                            ...
                            or entity.normalized_value.text
                            or "",
                            "type": entity.type_,
