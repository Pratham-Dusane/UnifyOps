...
class ComplianceService:
    ...
    def segment_regulatory_document(
        ...
        doc_entities = [
            e for e in store._entities.values() if e.document_id == document_id
        ]
        equipment_tags = [
            e.value for e in doc_entities if e.entity_type.value == "equipment_tag"
        ]

        for idx, text in enumerate(
            paragraphs[:10]
        ):  # Limit to top 10 clauses for simplicity
            ...
            match = re.search(
                r"(?:clause|section|rule|rule\s+no\.?)\s*(\d+(?:\.\d+)*\w*)",
                text,
                re.IGNORECASE,
            )
            if match:
            ...
            for other_doc in store._documents.values():
                if (
                    other_doc.doc_type == DocumentType.SAFETY_PROCEDURE
                    and other_doc.org_id == org_id
                ):
...
Regulation Text: {text}"""
    ...
    def generate_audit_package(
        self, org_id: str, user_uid: str, clause_ids: list[str]
    ) -> AuditPackageResponse:
        """
        ...
        for clause_id in clause_ids:
            ...
            package_markdown.append(
                f"## Regulatory Requirement: {clause.clause_number}"
            )
            ...
            if clause.linked_procedures:
                for proc_id in clause.linked_procedures:
                    ...
                    if proc:
                        ...
                        package_markdown.append(
                            f"- **Document**: {proc.original_filename}"
                        )
                        package_markdown.append("  - **Type**: Safety SOP")
                        package_markdown.append(
                            f"  - **Date**: {proc.created_at.date().isoformat()}"
                        )
                        ...
                        proc_chunks = [
                            ...
                            if c.document_id == proc.id
                        ]
                        if proc_chunks:
                            package_markdown.append(
                                f"  - **Procedure Excerpt (Citation: L1-L15)**:\n  > {proc_chunks[0].text[:300]}..."
                            )
            else:
                package_markdown.append(
                    "*Warning: No governing plant safety procedure is linked to this regulatory rule.*"
                )
            ...
            if open_gaps:
                package_markdown.append(
                    "🔴 **Non-Compliant: Active Deviation Flagged**"
                )
                ...
                package_markdown.append(
                    "- Governing procedure exists and is current. No active non-conformance indicators recorded."
                )
