...
class MaintenanceService:
        ...
        for doc in documents:
            ...
            events.append(
                TimelineEvent(
                    id=doc.id,
                    event_type=ev_type,
                    title=doc.original_filename.replace(".pdf", "")
                    .replace(".docx", "")
                    .replace("_", " "),
    ...
    def _extract_failure_mode(self, text: str) -> str | None:
        ...
        match = re.search(
            r"(?:failure mode|failed due to|failure|issue):\s*([^.\n]+)",
            text,
            re.IGNORECASE,
        )
        if match:
    ...
    def _extract_downtime(self, text: str) -> float | None:
        ...
        match = re.search(
            r"(\d+(?:\.\d+)?)\s*(?:hours|hrs|hr)\s*(?:of)?\s*downtime",
            text,
            re.IGNORECASE,
        )
        if match:
        ...
        match = re.search(
            r"downtime:\s*(\d+(?:\.\d+)?)\s*(?:hours|hrs|hr)", text, re.IGNORECASE
        )
        if match:
            return float(match.group(1))
        return None
    ...
    def get_needs_attention_list(
        ...
        for tag in equipment_tags:
            ...
            failure_count = sum(
                1
                for e in events
                if e.event_type == TimelineEventType.INCIDENT
                or e.failure_mode is not None
            )
            severity_count = sum(
                1 for e in events if e.event_type == TimelineEventType.INCIDENT
            )
            ...
            months_since_last = (
                datetime.now(timezone.utc) - last_service_date
            ).days / 30.44
            ...
            evidence_summary = self._synthesize_evidence_summary(
                tag, score, failure_count, avg_interval, months_since_last
            )
...
Create a precise explanation (max 150 characters) stating the risk factors (e.g. 'Equipment is overdue for service by X months based on a historical recurrence of Y months'). Do not use placeholders."""
        ...
        interval_text = (
            f" vs a historical interval of {interval:.1f} months" if interval else ""
        )
    ...
    def generate_rca_draft(
        ...
        if request_id:
            ...
            agent_bus.emit(
                request_id,
                "Ingestion Agent",
                f"Extracting context for equipment tag {equipment_tag}",
            )
...
JSON format only. Do not include markdown formatting code blocks like ```json."""
        ...
        draft = RCADraft(
            ...
            immediate_cause=rca_data.get(
                "immediate_cause", "Mechanical failure under investigation."
            ),
            five_whys=rca_data.get(
                "five_whys",
                [
            ...
            contributing_factors=rca_data.get(
                "contributing_factors",
                "No contributing factors identified in references.",
            ),
            corrective_actions=rca_data.get(
                "corrective_actions", "Lubricate bearings. Realign coupling."
            ),
